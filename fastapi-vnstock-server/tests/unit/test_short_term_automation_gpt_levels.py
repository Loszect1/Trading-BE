...
def test_refine_buy_levels_with_gpt_parses_valid_json(monkeypatch) -> None:
    monkeypatch.setattr(auto_svc.settings, "use_gpt", True)
    monkeypatch.setattr(
        auto_svc._automation_gpt_service,
        "generate_text_with_resilience",
        lambda **kwargs: (
            '{"entry_price": 10.5, "take_profit_price": 11.2, "stoploss_price": 9.9, "rationale": "ok"}'
        ),
    )
    out = auto_svc._refine_buy_levels_with_gpt(
...
def test_refine_buy_levels_with_gpt_rejects_invalid_constraints(monkeypatch) -> None:
    monkeypatch.setattr(auto_svc.settings, "use_gpt", True)
    monkeypatch.setattr(
        auto_svc._automation_gpt_service,
        "generate_text_with_resilience",
