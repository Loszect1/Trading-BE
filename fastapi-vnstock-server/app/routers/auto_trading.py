...
def post_demo_deposit(
    body: DemoDepositRequest,
    x_demo_session_id: Annotated[Optional[str], Header(alias="X-Demo-Session-Id")] = None,
) -> dict[str, Any]:
    session_id = _resolve_demo_session_id(x_demo_session_id)
    try:
        row = deposit_demo_cash(session_id=session_id, amount_vnd=body.amount_vnd)
        data = DemoDepositResponseData.model_validate(row)
        return {"success": True, "data": data.model_dump(mode="json")}
    except ValueError as exc:
        raise HTTPException(status_code=400, detail="Invalid deposit amount") from exc
    except Exception as exc:
        logger.exception("auto_trading.demo_deposit_failed", extra={"session_id": session_id})
        raise HTTPException(status_code=500, detail="Failed to deposit demo cash") from exc
